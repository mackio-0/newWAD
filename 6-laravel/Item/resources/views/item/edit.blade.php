            ...
            <form action="{{ route('item.update', $item->id) }}" method="post">
                ...
                <div class="grid gap-4 mb-3">
                    ...
                    <input type="text" id="name" name="name"
                        class=" bg-gray-50 border border-gray-800 rounded-md" value="{{ $item->name }}">
                    <label for="price" class=" block text-sm">Price</label>
                    <input type="text" id="price" name="price"
                        class=" bg-gray-50 border border-gray-800 rounded-md" value="{{ $item->price }}">
                    <label for="stock" class=" block text-sm">Stock</label>
                    <input type="text" id="stock" name="stock"
                        class=" bg-gray-50 border border-gray-800 rounded-md" value="{{ $item->stock }}">
                    <label for="description" class=" block text-sm">Description</label>
                    <textarea type="text" id="description" name="description" class=" bg-gray-50 border border-gray-800 rounded-md mb-3">{{ $item->description }}
                    </textarea>
                    <button type="submit"
