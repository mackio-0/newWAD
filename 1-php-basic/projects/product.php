                ...
                <div class="overflow-hidden">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            $productsJSON = array_filter(scandir($productInfoSaveFolder), fn ($el) => $el != "." && $el != "..");
                            // echo "<pre>";
                            // print_r($productsJSON);
                            foreach ($productsJSON as $product) :
                                $productDataJSON = file_get_contents($productInfoSaveFolder . "/" . $product);
                                // echo $productData;
                                $productObj = json_decode($productDataJSON);
                                // print($productObj->product_name);
                            ?>
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                                        <img src="<?= $productObj->product_photo; ?>" class=" w-10 h-10 object-cover object-center" alt="">
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                        <div class="flex gap-2">
                                            <?php for ($i = 1; $i <= 5; $i++) : ?>
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4 <?= $i <= $productObj->product_rating ? 'fill-yellow-500' : 'fill-gray-500' ?>">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                        <a href="./product-delete.php?productInfoFile=<?= $product; ?>&product_photo=<?= $productObj->product_photo; ?>" class="inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent text-blue-600 hover:text-blue-800 disabled:opacity-50 disabled:pointer-events-none dark:text-blue-500 dark:hover:text-blue-400 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
